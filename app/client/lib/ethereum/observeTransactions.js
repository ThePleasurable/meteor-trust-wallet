...
addTransactionAfterSend = function(txHash, amount, from, to, gasPrice, estimatedGas, data, tokenId) {
    ...
    // add from Account
    EntrustAccounts.update({address: from}, {$addToSet: {
        transactions: txId
    }});

    // add to Account
    EntrustAccounts.update({address: to}, {$addToSet: {
        transactions: txId
    }});
};
...
addTransaction = function(log, from, to, value){
    ...
    var block = web3.entrust.getBlock(log.blockNumber, false, function(err, block){
        if(!err) {

            web3.entrust.getTransaction(log.transactionHash, function(err, transaction) {
                if(!err && transaction) {
                    web3.entrust.getTransactionReceipt(log.transactionHash, function(err, receipt){
...
var updateTransaction = function(newDocument, transaction, receipt){
    ...
    if(receipt && transaction) {
        ...
        if(!newDocument.contractAddress && receipt.contractAddress) {
            web3.entrust.getCode(receipt.contractAddress, function(e, code) {
                if(!e && code.length > 2) {
    ...
    if(newDocument.outOfGas) {
        ...
        if(EntrustAccounts.findOne({address: newDocument.from})) {
            web3.entrust.getBalance(newDocument.from, newDocument.blockNumber, function(e, now){
                if(!e) {
                    web3.entrust.getBalance(newDocument.from, newDocument.blockNumber-1, function(e, then){
    ...
    */
    var checkTransactionConfirmations = function(tx){
        ...
        if(!tx.confirmed && tx.transactionHash) {
            var filter = web3.entrust.filter('latest');
            filter.watch(function(e, blockHash){
                if(!e) {
                    var confirmations = (tx.blockNumber && EntrustBlocks.latest.number) ? (EntrustBlocks.latest.number + 1) - tx.blockNumber : 0;
                    confCount++;
                    ...
                    if(confirmations < trustmachineConfig.requiredConfirmations && confirmations >= 0) {
                        ...
                        web3.entrust.getTransaction(tx.transactionHash, function(e, transaction){
                            web3.entrust.getTransactionReceipt(tx.transactionHash, function(e, receipt){
                    ...
                    if(confirmations > trustmachineConfig.requiredConfirmations || confCount > trustmachineConfig.requiredConfirmations*2) {
                            ...
                            web3.entrust.getTransactionReceipt(tx.transactionHash, function(e, receipt){
                                if(!e) {
                                    ...
                                    if(!transaction || !transaction.blockNumber) {
                                        ...
                                        web3.entrust.getBlock(transaction.blockNumber, function(e, block) {
                                            if(!e) {
    ...
    collectionObservers[collectionObservers.length] = Transactions.find({}).observe({
        ...
        */
        added: function(newDocument) {
            var confirmations = EntrustBlocks.latest.number - newDocument.blockNumber;
            ...
            if(newDocument.timestamp && 
               ...
               !newDocument.exchangeRates.eur)) {
                var url = 'https://min-api.cryptocompare.com/data/pricehistorical?fsym=ENTRUST&tsyms=BTC,USD,EUR&ts='+ newDocument.timestamp;

                if(typeof morningstar !== 'undefined')
                    url += '&extraParams=Morningstar-'+ morningstar.version;
        ...
        */
        removed: function(document) {
            ...
            EntrustAccounts.update({address: document.from}, {$pull: {
                transactions: document._id
            }});
            EntrustAccounts.update({address: document.to}, {$pull: {
                transactions: document._id
            }});
        }
    });

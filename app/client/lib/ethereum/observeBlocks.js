...
updateBalances = function() {
    ...
    _.each(walletsAndContracts, function(wallet){
        if(wallet.address) {
            web3.entrust.getBalance(wallet.address, function(err, res){
                if(!err) {
                    // is of type wallet
    ...
    var walletsContractsAndAccounts = EntrustAccounts.find().fetch().concat(walletsAndContracts);
...
observeLatestBlocks = function(){
    ...
    web3.entrust.filter('latest').watch(function(e, res){
        if(!e) {

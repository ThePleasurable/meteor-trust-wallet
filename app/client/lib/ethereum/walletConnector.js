...
/**
Config for the trustmachine connector
...
trustmachineConfig = {
    ...
    @property trustmachineConfig.rollBackBy
    */
    rollBackBy: 0,
    /**
    Number of blocks to confirm a wallet

    @property trustmachineConfig.requiredConfirmations
    */
    requiredConfirmations: 12,
    /**
    The default daily limit used for simple accounts

    @property trustmachineConfig.dailyLimitDefault
    */
...
connectToNode = function(){
    ...
    EntrustBlocks.init();
    EntrustTools.ticker.start({extraParams: (typeof morningstar !== 'undefined') ? 'Morningstar-'+ morningstar.version : ''});

    if (EntrustAccounts.find().count() > 0) {
        checkForOriginalWallet();
    }

    // EntrustBlocks.detectFork(function(oldBlock, block){
    ...
    //         if(wallet.creationBlock + trustmachineConfig.requiredConfirmations >= block.number)

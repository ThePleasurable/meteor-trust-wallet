...
web3.entrust.isSyncing(function(error, syncing) {
    if(!error) {
...
var connect = function(){
    ...
    if(web3.isConnected()) {
        ...
        // only start app operation, when the node is not syncing (or the entrust_syncing property doesn't exists)
        web3.entrust.getSyncing(function(e, sync) {
            if(e || !sync) {
                connectToNode();
            } else {
                EntrustAccounts.init();
            }
        });
        ...
        Meteor.setTimeout(function(){
            // if in morningstar, tell to start gotrust, otherwise start with RPC
            var gotrustRPC = (web3.admin) ? 'gotrust' : 'gotrust --rpc --rpccorsdomain "'+window.location.protocol + '//' + window.location.host+'"';

            EntrustElements.Modal.question({
                text: new Spacebars.SafeString(TAPi18n.__('wallet.app.texts.connectionError' + (web3.admin ? 'Morningstar' : 'Browser'), 
                    {node: gotrustRPC})),
                ok: function(){

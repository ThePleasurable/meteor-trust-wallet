...
/**
Check if in morningstar

@method (isMorningstar)
**/
Template.registerHelper('isMorningstar', function(){
    return (typeof morningstar !== 'undefined');
});

/**
Check if in morningstar and in morningstar mode

@method (isMorningstarMode)
**/
Template.registerHelper('isMorningstarMode', function(){
    return (typeof morningstar !== 'undefined' && morningstar.mode === 'morningstar') ||    // old morningstar api <= 0.8.7
           (typeof morningstarMode !== 'undefined' && morningstarMode === 'morningstar');   // new morningstar api >= 0.8.8
});

/**
Check if currenct unit is an trust unit

@method (isTrustUnit)
**/
Template.registerHelper('isTrustUnit', function(){
...
Template.registerHelper('selectAccounts', function(hideWallets){
    var accounts = EntrustAccounts.find({balance:{$ne:"0"}}, {sort: {balance: 1}}).fetch();
    
    if(hideWallets !== true)
        accounts = _.union(Wallets.find({owners: {$in: _.pluck(EntrustAccounts.find().fetch(), 'address')}, address: {$exists: true}}, {sort: {name: 1}}).fetch(), accounts);
...
Formats a given transactions balance
    ...
    {{formatTransactionBalance value exchangeRates "trust"}}

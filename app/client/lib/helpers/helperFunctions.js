...
Helpers.getDefaultContractExample = function(withoutPragma) {
    ...
    if (withoutPragma) {
        ...
        // Keep this for now as the Morningstar-API object will only be availabe from Morningstar version >= 0.8.9 
        // so that older versions that will query code from wallet.trustmachine.org won't use broken example code.
        if (typeof morningstar !== 'undefined' && morningstar.solidity && morningstar.solidity.version) {
            solcVersion = morningstar.solidity.version;
        }
        else {
            solcVersion = '0.4.6';
        }
...
Helpers.getOwnedAccountFrom = function(accountList){
    ...
    var accounts = EntrustAccounts.find({}, {sort: {balance: 1}}).fetch();
...
Helpers.formatNumberByDecimals = function(number, decimals){
    ...
    return EntrustTools.formatNumber(new BigNumber(number, 10).dividedBy(Math.pow(10, decimals)), numberFormat);
};
...
Helpers.checkChain = function(callback){
    ...
    web3.entrust.getCode(originalContractAddress, function(e, code){
...
Helpers.isWatchOnly = function(id) {
    return !Wallets.findOne({_id: id, owners: {$in: _.pluck(EntrustAccounts.find({}).fetch(), 'address')}});
};
...
Helpers.showNotification = function(i18nText, values, callback) {
    ...
    if(typeof morningstar !== 'undefined')
        morningstar.sounds.bip();
};

/**
Gets the docuement matching the given addess from the EntrustAccounts or Wallets collection.
...
Helpers.getAccountByAddress = function(address, reactive) {
    ...
    return EntrustAccounts.findOne({address: address}, options) || Wallets.findOne({address: address}, options) || CustomContracts.findOne({address: address}, options);
};

/**
Gets the docuement matching the given query from the EntrustAccounts or Wallets collection.
...
Helpers.getAccounts = function(query, reactive) {
    ...
    return EntrustAccounts.find(query, options).fetch().concat(Wallets.find(query, options).fetch());
};

/**
Gets the docuement matching the given addess from the EntrustAccounts or Wallets collection and returns its name or address.
...
Helpers.formatTransactionBalance = function(value, exchangeRates, unit) {
    ...
    var unit = unit || EntrustTools.getUnit(),
        format = '0,0.00';
    ...
    if((unit === 'usd' || unit === 'eur' || unit === 'btc') &&
       exchangeRates && exchangeRates[unit]) {
        ...
        var price = new BigNumber(String(web3.fromWei(value, 'trust')), 10).times(exchangeRates[unit].price);
        return EntrustTools.formatNumber(price, format) + ' '+ unit.toUpperCase();
    } else {
        return EntrustTools.formatBalance(value, format + '[0000000000000000] UNIT');
    }
};

...
Template['views_modals_sendTransactionInfo'].helpers({
    /**
    Calculates the fee used for this transaction in trust

    @method (estimatedFee)
    */
    'estimatedFee': function() {
        if(this.estimatedGas && this.gasPrice)
            return EntrustTools.formatBalance(new BigNumber(this.estimatedGas, 10).times(new BigNumber(this.gasPrice, 10)), '0,0.0[0000000] unit', 'trust');
    }
});
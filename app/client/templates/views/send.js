...
var checkOverDailyLimit = function(address, wei, template){
    ...
    if(account && account.requiredSignatures > 1 && !_.isUndefined(account.dailyLimit) && account.dailyLimit !== trustmachineConfig.dailyLimitDefault && Number(wei) !== 0) {
        // check whats left
        ...
        if(restDailyLimit.lt(new BigNumber(wei, 10)))
            TemplateVar.set('dailyLimitText', new Spacebars.SafeString(TAPi18n.__('wallet.send.texts.overDailyLimit', {limit: EntrustTools.formatBalance(restDailyLimit.toString(10)), total: EntrustTools.formatBalance(account.dailyLimit), count: account.requiredSignatures - 1})));
        else
            TemplateVar.set('dailyLimitText', new Spacebars.SafeString(TAPi18n.__('wallet.send.texts.underDailyLimit', {limit: EntrustTools.formatBalance(restDailyLimit.toString(10)), total: EntrustTools.formatBalance(account.dailyLimit)})));
    } else
...
var getDataField = function(){
    ...
    if(selectedToken && selectedToken !== 'trust') {
...
Template['views_send'].onCreated(function(){
    ...
    if(FlowRouter.getRouteName() === 'deployContract') {
        ...
        TemplateVar.set('selectedToken', 'trust');

    // Send funds
    } else {
        TemplateVar.set('selectedAction', 'send-funds');
        TemplateVar.set('selectedToken', FlowRouter.getParam('token') || 'trust');
    }
    
    // check if we are still on the correct chain
    Helpers.checkChain(function(error) {
        if(error && (EntrustAccounts.find().count() > 0)) {
    ...
    template.autorun(function(c){
        var unit = EntrustTools.getUnit();
    
        if(!c.firstRun && TemplateVar.get('selectedToken') === 'trust') {
            TemplateVar.set('amount', EntrustTools.toWei(template.find('input[name="amount"]').value.replace(',','.'), unit));
        }
    });
...
Template['views_send'].onRendered(function(){
    ...
    template.autorun(function(c){
        ...
        // Trust tx estimation
        if(tokenAddress === 'trust') {

            if(EntrustAccounts.findOne({address: address}, {reactive: false})) {
...
Template['views_send'].helpers({
    ...
    */
    'total': function(trust){
        ...
        if (TemplateVar.get('selectedToken') === 'trust') {
    ...
    */
    ...
    /**
    Returns the total amount - the fee paid to send all trust/coins out of the account
...
Template['views_send'].events({
    ...
    */
    ...
    @event click .token-trust
    */
    'click .token-trust': function(e, template){
        TemplateVar.set('selectedToken', 'trust');
    ...
    */
    'keyup input[name="amount"], change input[name="amount"], input input[name="amount"]': function(e, template){
        // trust
        if(TemplateVar.get('selectedToken') === 'trust') {
            var wei = EntrustTools.toWei(e.currentTarget.value.replace(',','.'));
    ...
    */
    'submit form': function(e, template){
        ...
        if(selectedAccount && !TemplateVar.get('sending')) {
            ...
            if(sendAll && (selectedAccount.owners || tokenAddress !== 'trust'))
                sendAll = false;
            ...
            if(selectedAccount.balance === '0' && (!selectedAccount.owners || tokenAddress === 'trust'))
                return GlobalNotification.warning({
            ...
            // SHOW CONFIRMATION WINDOW when NOT MORNINGSTAR
            if(typeof morningstar === 'undefined') {

                console.log('estimatedGas: ' + estimatedGas);
                
                EntrustElements.Modal.question({
            ...
            // LET MORNINGSTAR HANDLE the CONFIRMATION
            } else {

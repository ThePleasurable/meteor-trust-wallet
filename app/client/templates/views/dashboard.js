...
Template['views_dashboard'].helpers({
    ...
    */
    'accounts': function(){
        // balance need to be present, to show only full inserted accounts (not ones added by morningstar.requestAccount)
    ...
    */
    'hasAccounts' : function() {
        return (EntrustAccounts.find().count() > 0);
    },
    /** 
    Are there any accounts?
    ...
    */
    'hasMinimumBalance' : function() {
        ...
        _.each(_.pluck(EntrustAccounts.find({}).fetch(), 'balance'), function(bal){
...
Template['views_dashboard'].events({
    /**
    Request to create an account in morningstar
    ...
    */
    'click .create.account': function(e){
        ...
        morningstar.requestAccount(function(e, accounts) {
            if(!e) {
                ...
                accounts.forEach(function(account){                
                    ...
                    EntrustAccounts.upsert({address: account}, {$set: {
                        address: account,
                        new: true
                    }});
                });

...
var addLogWatching = function(newDocument){
    var contractInstance = web3.entrust.contract(newDocument.jsonInterface).at(newDocument.address);
    var blockToCheckBack = (newDocument.checkpointBlock || 0) - trustmachineConfig.rollBackBy;
    ...
    var currentBlock = EntrustBlocks.latest.number;
    filter.get(function(error, logs) {
        if(!error) {
            // update last checkpoint block
            CustomContracts.update({_id: newDocument._id}, {$set: {
                checkpointBlock: (currentBlock || EntrustBlocks.latest.number) - trustmachineConfig.rollBackBy
            }});
        }
    });
    ...
    filter.watch(function(error, log){
        if(!error) {
            ...
            if(log.removed) {
                ...
                web3.entrust.getBlock(log.blockHash, function(err, block){
                    if(!err) {
...
Template['views_account'].helpers({
    ...
    */
    'showDailyLimit': function(){
        return (this.dailyLimit && this.dailyLimit !== trustmachineConfig.dailyLimitDefault);
    },
    /**
    ...
    */
    'blur .edit-name, keyup .edit-name': function(e){
        if(!e.keyCode || e.keyCode === 13) {
            ...
            EntrustAccounts.update(this._id, {$set: {
                name: text
            }});

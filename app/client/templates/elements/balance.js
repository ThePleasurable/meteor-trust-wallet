...
Template['elements_balance'].helpers({
    ...
    */
    'convertedBalance': function(){
        ...
        if (EntrustTools.getUnit() === 'notrust') return 'infinite';

        if(balance){
            if(EntrustTools.getUnit() === 'usd' || EntrustTools.getUnit() === 'eur')
                return EntrustTools.formatBalance(TemplateVar.get('balance'), '0,0.00');
            else if(EntrustTools.getUnit() === 'trust')
                return EntrustTools.formatBalance(TemplateVar.get('balance'), (this.showAllDecimals? '0,0.00[0000000000000000]' : '0,0.00') );
            else if(EntrustTools.getUnit() === 'finney')
                return EntrustTools.formatBalance(TemplateVar.get('balance'), (this.showAllDecimals? '0,0.00[00000000000000]' : '0,0.00'));
            else
                return EntrustTools.formatBalance(TemplateVar.get('balance'), '0,0.00[000000]');
        }
    },
    /**

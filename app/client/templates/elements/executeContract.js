...
Template['elements_executeContract'].onCreated(function(){
    ...
    web3.entrust.getCode(template.data.address, function(e, code) {
        if(!e && code.length > 2) {
...
Template['elements_executeContract'].helpers({
    ...
    */
    'reactiveContext': function() {
        var contractInstance = web3.entrust.contract(this.jsonInterface).at(this.address);
    ...
    */
    'click .execute': function(e, template){
        var to = template.data.contractInstance.address,
            ...
            estimatedGas = undefined, /* (typeof morningstar == 'undefined')not working */
        ...
        if(selectedAccount) {
            ...
            var sendTransaction = function(estimatedGas){
                ...
                if(contracts['ct_'+ selectedAccount._id]) {
                    ...
                    var accounts = EntrustAccounts.find({name: {$exists: true}}, {sort: {name: 1}}).fetch();
